...
(function () {
  ...
  player.startDelay = 0;
  player.BPM = 120;
  player.listeners = []; // eslint-disable-next-line no-multi-assign
  ...
  player.removeListener = function (listener) {
    if (listener) {
      var index = player.listeners.indexOf(listener);
      ...
      switch (event.subtype) {
        case 'controller':
          eventQueue.push({
            event: event,
            time: queueTime,
            source: _root__WEBPACK_IMPORTED_MODULE_1___default.a.setController(channelId, event.controllerType, event.value, delay),
            interval: scheduleTracking(channelId, event.controllerType, queuedTime + player.startDelay, offset - foffset, 176, event.value)
          });
          break;
